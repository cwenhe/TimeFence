...
发布日志默认限制详细度。日志只应包含规则标识、应用包名、时间边界、闹钟处理和权限状态，不应包含屏幕文字、输入内容、账号、密码或通知正文。提交日志前仍应人工检查并移除不希望公开的设备信息。
...
规则保存在应用私有数据库中，诊断状态和待补检标记保存在应用私有目录。卸载应用会删除这些本地数据。项目不提供云端诊断上传；是否分享日志由用户自行决定。
